...
Declare `openid`, `email`, `https://www.googleapis.com/auth/gmail.modify` in Google Auth Platform Data Access. Existing read-only or send-only connections must reconnect to grant message management permission before marking messages as read. Use Production audience for public access and complete the applicable Google verification; reading is a restricted scope. See [GOOGLE-LAUNCH.md](GOOGLE-LAUNCH.md).
...
- Opening a message leaves Gmail read status unchanged. More > Mark all as read removes UNREAD from every matching message in the current folder/category/search, in batches across all pages. Selecting checkboxes changes this to Mark selected as read. Open in Gmail provides mailbox management. Reading or rendering an email never sends one.
