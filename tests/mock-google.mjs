...
const scope = "openid email https://www.googleapis.com/auth/gmail.modify";
...
globalThis.fetch = async (input, options = {}) => {
  ...
  if (route.startsWith("labels/")) {
    const label = labels.find(label => label.id === decodeURIComponent(route.slice(7))) || {};
    if (label.id === "INBOX" && markedRead.size >= 501) label.messagesUnread = 0;
    return json(label);
  }
  if (route === "messages/batchModify") {
    const body = JSON.parse(options.body);
    if (options.method !== "POST" || JSON.stringify(body.removeLabelIds) !== '["UNREAD"]' || !body.ids.length || body.ids.length > 1000) throw new Error("Invalid mark-read operation");
    body.ids.forEach(id => markedRead.add(id));
    return new Response(null, { status: 204 });
  }
  if (route === "messages") {
    if (url.searchParams.getAll("labelIds").includes("UNREAD")) {
      const unread = Array.from({ length: 501 }, (_, i) => ({ id: (0xabc123 + i).toString(16) })).filter(message => !markedRead.has(message.id));
      return json({ messages: unread.slice(0, 500), nextPageToken: unread.length > 500 ? "shifted-page" : undefined });
    }
  ...
  if (route.startsWith("messages/") && !route.endsWith("/send")) {
    ...
    return json({ id, threadId: "abc999", labelIds: ["INBOX", ...(markedRead.has(id) ? [] : ["UNREAD"]), "STARRED"], internalDate: "1788825600000", snippet: "Actual &amp; escaped snippet", payload: { headers, mimeType: "multipart/mixed", parts: [
